...
int main(int argc, char* argv[])
{
    ...
    if(cuda_ret != cudaSuccess) FATAL("Unable to allocate device variables");
    stopTime(&timer); printf("%f s\n", elapsedTime(timer));
    ...
    if(cuda_ret != cudaSuccess) FATAL("Unable to copy mask to device constant memory");
    ...
    //INSERT CODE HERE
    dim3 dim_grid(((P_d.width - 1)/TILE_SIZE) + 1, ((P_d.height - 1)/TILE_SIZE) + 1);
    ...
    cuda_ret = cudaDeviceSynchronize();
  	if(cuda_ret != cudaSuccess) FATAL("Unable to copy device variables from host");
    stopTime(&timer); printf("%f s\n", elapsedTime(timer));
